...
public class ProductRepositoryImpl implements ProductRepositoryCustom {
    ...
    private static final String SQL_GET_ALL_ACTIVE_CUSTOM = "select p.*, uhp.user_id, uhp.activation_date, uhp.active from user_has_product uhp join product p on uhp.product_id = p.id where uhp.active = 1";
    private static final String SQL_GET_ACTIVE_BY_USER_ID_CUSTOM = "select p.*, uhp.user_id, uhp.activation_date, uhp.active from user_has_product uhp join product p on uhp.product_id = p.id where uhp.active = 1 and uhp.user_id = ?";
